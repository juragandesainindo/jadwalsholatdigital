@extends('admin.layout.master')
@section('title','Keuangan Masjid')

@section('content')
<nav class="navbar navbar-expand navbar-light navbar-bg">
    <a class="sidebar-toggle js-sidebar-toggle">
        <i class="hamburger align-self-center"></i>
    </a>

    <div class="navbar-collapse collapse">
        <ul class="navbar-nav navbar-align">
            <li class="nav-item dropdown">


            </li>
            <li class="nav-item dropdown">
                <span class="text-dark text-uppercase">
                    @yield('title')
                </span>

            </li>
        </ul>
    </div>
</nav>

<main class="content">
    <div class="container-fluid p-0">

        <div class="row">
            <div class="col-12 col-lg-12">
                <h1 class="h3 mb-3 text-dark text-capitalize">
                    <a href="{{ route('keuangan-masjid.create') }}" class="btn btn-primary">
                        <i class="align-middle me-2" data-feather="plus-circle"></i>
                        Tambah @yield('title')
                    </a>
                </h1>

                @include('admin.message')

                @forelse ($kategoris as $kategori)
                <div class="card">
                    <div class="card-body">
                        <h4 class="text-capitalize text-black"><strong>{{ $kategori->kategori }}</strong></h4>
                        <div class="table-responsive">

                            <table id="datatable" class="table table-bordered" style="width: 100%">
                                <thead class="bg-dark text-white">
                                    <tr class="text-center">
                                        <th>No</th>
                                        <th>Tanggal</th>
                                        <th>Keterangan</th>
                                        <th>Jenis</th>
                                        <th>Nominal (Rp)</th>
                                        <th>Status</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="text-dark">
                                    @foreach ($kategori->keuanganMasjid as $key => $item)
                                    <tr>
                                        <td class="text-center">{{ ++$key }}</td>
                                        <td>{{ $item->tanggal }}</td>
                                        <td>{{ $item->keterangan }}</td>
                                        <td class="text-center">
                                            @if ($item->jenis == 'Masuk')
                                            <span class="badge bg-primary">Masuk</span>
                                            @else
                                            <span class="badge bg-danger">Keluar</span>

                                            @endif
                                        </td>
                                        <td class="text-end">{{ number_format($item->nominal) }}</td>
                                        <td class="text-center">
                                            @if ($item->status == 1)
                                            <span class="badge bg-success">Aktif</span>
                                            @else
                                            <span class="badge bg-secondary">Tidak Aktif</span>

                                            @endif
                                        </td>
                                        <td class="text-center">
                                            <a class="btn btn-info btn-sm mb-1"
                                                href="{{ route('keuangan-masjid.edit',$item->id) }}">
                                                <i class="align-middle me-2" data-feather="edit"></i>
                                                Edit
                                            </a>
                                            <button type="button" class="btn btn-danger btn-sm mb-1"
                                                data-bs-toggle="modal" data-bs-target="#delete-{{ $item->id }}">
                                                <i class="align-middle me-2" data-feather="trash"></i>
                                                Hapus
                                            </button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr class="text-dark">
                                        <th colspan="4">Jumlah Nominal</th>
                                        <th class="text-end">{{
                                            number_format($kategori->keuanganMasjid->sum('nominal')) }}</th>
                                        <td colspan="2"></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    </div>
                </div>
                @empty
                <div class="card">
                    <div class="card-body">
                        Kategori Keuangan belum tersedia. Silahkan klik link ini <a
                            href="{{ route('kategori-keuangan.index') }}">Kategori Keuangan</a>
                    </div>
                </div>
                @endforelse
            </div>
        </div>

    </div>
</main>

<!-- Modal -->
@foreach ($kategoris as $kategori)
@foreach ($kategori->keuanganMasjid as $key => $item)
<div class="modal fade" id="delete-{{ $item->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">Hapus @yield('title')</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('keuangan-masjid.destroy',$item->id) }}" method="post">
                @csrf
                @method('DELETE')
                <div class="modal-body text-dark">
                    Apakah Yakin Ingin Menghapus Keuangan ini? <br>
                    <strong>Keterangan : {{ $item->keterangan }}</strong> <br>
                    <strong>Nominal : Rp. {{ $item->nominal }}</strong>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-danger">
                        <i class="align-middle me-2" data-feather="trash"></i>
                        Ya, hapus
                    </button>
                </div>
            </form>
        </div>
    </div>
</div>
@endforeach
@endforeach


@endsection

@push('js')
<script>
    function formatRupiah(input) {
            let angka = input.value.replace(/\D/g, ""); // Hanya angka
            let formatted = angka.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
            input.value = formatted;
        }
</script>
@endpush